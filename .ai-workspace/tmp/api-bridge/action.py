...
DEFAULTS = [
    ("photo", "拍照", "拍照动作", "other", "", "", 0, 1,0),
    ("stand_down", "趴下", "底盘动作（需运行时支持）", "chassis", "", "", 0, 1,1),
    ("sit_down", "坐下", "底盘动作（需运行时支持）", "chassis", "", "", 0, 1,2),
    ("hello", "打招呼", "底盘动作（需运行时支持）", "chassis", "", "", 0, 1,3),
    ("stretch", "伸展", "底盘动作（需运行时支持）", "chassis", "", "", 0, 1,4),
    ("stand_up", "恢复站立", "底盘动作（需运行时支持）", "chassis", "", "", 0, 1,5),
]
...
def add_action(req: NavigationActionAddRequest):
    with get_conn() as conn:
        _ensure_table(conn)
        try:
            cur = conn.execute("INSERT INTO NavigationAction(code,name,category,description,content,parameters,duration,enabled,sort) VALUES (?,?,?,?,?,?,?,?,?)", (req.code,req.name,req.category,req.description or '',getattr(req,'content',''),getattr(req,'parameters',''),getattr(req,'duration',0) or 0,req.enabled,99))
