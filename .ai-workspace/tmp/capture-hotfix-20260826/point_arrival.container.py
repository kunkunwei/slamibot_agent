...
精简版：容器未部署 tts_service，只接通 photo（拍照）链路；
tts 播报记录日志提示，不阻断其它动作。
"""
...
class PointArrivalDispatcher:
    """Deduplicate one ROS event, then fan out photo action."""
    ...
    def handle(self, event: dict[str, Any]) -> None:
        ...
        if action == "photo":
            ...
            LOGGER.info(
                "point action tts (content=%r) skipped: 容器未部署 TTS 服务",
                content,
            )
