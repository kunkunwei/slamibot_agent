...
class RosbridgeClient:
    ...
    def stop(self) -> None:
        for topic in self._topics:
            try:
                ...
                LOGGER.warning("ros_client: 取消 topic 订阅失败: %s", exc)
        self._topics = []
    ...
    def cancel_move_base_goals(self) -> Tuple[bool, str]:
        """Publish an empty GoalID to cancel every move_base action goal.

        This is an independent best-effort fallback for cases where the
        nav_multi service is wedged but rosbridge is still responsive.
        """
        if not self.is_connected:
            return False, "rosbridge 未连接"
        try:
            topic = roslibpy.Topic(
                self._ros, "/move_base/cancel", "actionlib_msgs/GoalID"
            )
            topic.publish(roslibpy.Message({"stamp": {"secs": 0, "nsecs": 0}, "id": ""}))
            return True, "已取消全部 move_base goal"
        except Exception as exc:
            return False, "发布 /move_base/cancel 失败: %s" % exc
    ...
    def get_latest_frame(self) -> Optional[bytes]:
        """Backward-compatible frame accessor used by older capture callers."""
        frame, _received_at = self.get_latest_frame_snapshot()
        return frame
