...
class TeleopController:
    ...
    def force_enable(self) -> ApiResponse:
        """Enable teleop after the coordinator has cancelled navigation.

        Unlike ``enable()``, this path never pauses or resumes an old task.
        It is intentionally used only by the atomic manual-takeover flow.
        """
        from .base_mode import base_mode_manager
        ...
        with self._lock:
            ...
            self._watchdog_triggered = False
            self._active = False
        return ok(self._status_payload(), "teleop 已强制启用")

    def disable(
        self,
        resume_navigation: bool = True,
        claim_auto: bool = True,
    ) -> ApiResponse:
        """Disarm, send zero and optionally resume navigation.

        ``resume_navigation=False`` is required by mapping/manual safety
        transitions: an old paused task must never be revived there.
        ...
        try:
            with self._lock:
                ...
                need_resume = self._nav_was_paused and resume_navigation
                self._enabled = False
                self._nav_was_paused = False
                self._last_msg_time = None
                self._last_linear = 0.0
        ...
        if claim_auto:
            try:
                from .control_ownership import OWNER_AUTO, claim
                claim(OWNER_AUTO)
            except Exception as exc:
                LOGGER.warning("teleop: 更新自动控制权失败: %s", exc)
