...
class RosbridgeClient:
    ...
    def __init__(self) -> None:
        ...
        self._latest_pose_msg: Optional[dict[str, Any]] = None
    ...
    def _hold_robot_map_pose(self, seconds: float = 10.0) -> None:
        """后台线程：每 0.5s 把最新位姿发布到 /robot_map_pose，持续 seconds 秒。

        手动标点/重定位后 AMCL 静止时不会持续回发 /amcl_pose，用此保持 3D 箭头可见。
        """
        def _loop() -> None:
            deadline = time.monotonic() + seconds
            while time.monotonic() < deadline:
                with self._lock:
                    msg = self._latest_pose_msg
                if msg is not None:
                    self._publish_robot_map_pose(msg)
                time.sleep(0.5)
    ...
    def set_initial_pose(
        ...
        try:
            ...
            # 手动标点：立即把新位姿显示到 3D 箭头并持续 10s（AMCL 静止时不回发 /amcl_pose）
            with self._lock:
                self._latest_pose_msg = message
            self._publish_robot_map_pose(message)
            self._hold_robot_map_pose(10.0)
    ...
    def trigger_global_localization(self) -> Tuple[bool, str]:
        ...
        if success:
            # 点击重定位：持续发布当前位姿 10s，3D 箭头随 AMCL 收敛实时更新
            self._hold_robot_map_pose(10.0)
