...
_ARRIVE_SETTLE_S = max(
    0.0,
    float(os.environ.get("NAV_MULTI_ARRIVE_SETTLE_S", "0.5")),
)
...
def _publish_point_arrived(point):
    """Publish action metadata; side effects run only in the FastAPI process."""
    if _ARRIVE_SETTLE_S > 0:
        rospy.sleep(rospy.Duration.from_sec(_ARRIVE_SETTLE_S))
    event = {
        "runId": _run_id,
        "arrivalId": uuid.uuid4().hex,
        "taskId": _task_id,
        "taskName": _task_name,
...
def _handle_execute(req):
    ...
    """
    global _state, _task_id, _task_name, _run_id, _task_points, \
