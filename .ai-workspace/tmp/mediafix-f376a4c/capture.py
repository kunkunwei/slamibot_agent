...
def _mjpeg_frames(initial_frame: bytes, initial_received_at: float) -> Iterator[bytes]:
    """Yield latest-only JPEG snapshots at a bounded rate without re-encoding."""
    fps = max(1.0, min(float(os.environ.get("CAMERA_STREAM_FPS", "8")), 30.0))
    interval = 1.0 / fps
    max_age = _camera_frame_max_age_s()
    frame = initial_frame
    received_at = initial_received_at
...
def stream_camera() -> StreamingResponse:
    """Stream the shared compressed-camera cache as latest-only MJPEG."""
    frame, received_at = ros_client.get_latest_frame_snapshot()
    if frame is None or received_at is None:
    ...
    return StreamingResponse(
        _mjpeg_frames(frame, received_at),
