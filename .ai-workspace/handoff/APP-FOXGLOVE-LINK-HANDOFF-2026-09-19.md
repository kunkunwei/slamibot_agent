...
把 APP 的 ROS 链路从「rosbridge JSON + cbor-raw 专属 socket」换成 **Foxglove WebSocket 协议**（D360 走 `ros1` 二进制编码），视频统一到一个 HTTP MJPEG 端点；删掉所有旧协议/旧端点代码，不留兼容层。最终同一份 APP 既能连 D360（ROS1）也能连 D360S（ROS2）。
...
| 视频 | **唯一**端点 `http://<host>:5010/api/camera/preview.mjpeg`（三路拼接 B,A,C、10fps、无客户端时不合成；`/api/camera/preview.status` 报 clients/fps） |
...
| `NativeNavigationSession.kt` | 455 | nav 会话（MjpegStreamClient + rosbridge） | 同上 |
| `NativeDataCollectionSession.kt` | 592 | 采集：点云 + FAST_LIO 位姿 + **`/keyframe` fallback** + MJPEG 预览 | 删 fallback；视频只留 5010 |
| `MjpegStreamClient.kt` | 208 | HTTP MJPEG（OkHttp） | **保留**，URL 统一到 5010 |
...
| `RobotEndpoint.kt` | 62 | 端口常量：9090/5000/5001/5010 + `cameraStreamUrl`(5000)/`videoLinkCameraStreamUrl`/`cameraPreviewUrl`(5010) | **删 5000 两条**，只留 5010 |
...
- 所以"APP 只用一个视频端点"这条对 D360S **暂不成立**。要让两个产品共用同一份视频契约、并让 APP 只保留一条视频路径，需要给 D360S 也做一个等价端点（相机节点内嵌 HTTP MJPEG/H.264，端口与路径与 D360 对齐，例如同样 5010 + `/api/camera/preview.mjpeg`），并把 D360S 控制台的三路订阅删掉。D360S 尚未发布，可以按工程规范直接改。
