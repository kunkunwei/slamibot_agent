...
class BaseModeManager:
    ...
    默认状态: ``mode=NONE``, ``ready=True``。NONE 是安全可用状态,
    启动 API 后无需任何额外配置即可正常工作;切换到 SCOUT/GO2 失败
        ...
        * ``_managed_scout`` 句柄存在且进程仍在运行:进程一旦已退出
        ...
        with self._lock:
            ...
            if record is None or record.proc.poll() is not None:
                # 句柄缺失或进程已退出:fail closed,标记 not ready + 明确 reason。
                self._ready = False
    ...
    def switch(self, mode: str) -> ApiResponse:
        ...
        target = str(mode or "").strip().upper()
    ...
    def _switch_to_scout_locked(self) -> ApiResponse:
        ...
        if out:
            ...
            if "/scout_base_node" in nodes:
                self._mode = MODE_NONE
                self._ready = False
                self._reason = "SCOUT_NODE_ALREADY_RUNNING"
                return fail(
                    "检测到已有 /scout_base_node, 请先停掉外部 Scout 驱动"
                )
